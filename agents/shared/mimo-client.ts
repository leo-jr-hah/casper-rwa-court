/**
 * LLM Client: Groq + Heuristic Fallback
 * 
 * Uses Groq (llama-3.3-70b-versatile) as the primary LLM.
 * Falls back to heuristic if Groq fails.
 */
...
function buildFallbackResponse(userPrompt: string): any {
  ...
  return {
    fallback: true,
    fallbackReason: 'Groq unavailable. Using heuristic response',
...
// ─── Main Export: askJuror (Groq + Fallback chain) ──────────────────────────

/**
 * Ask the LLM juror a question and get a structured JSON response.
 * 
 * Chain: Groq → Heuristic Fallback
 * 
 * Each step has a 15s timeout. If Groq fails, a deterministic heuristic response is returned.
 */
